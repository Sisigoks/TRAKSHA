...
with the package (about 2.5 MB) so `traksha preflight` works on a fresh install
with no network. `scripts/make_test_fixture.py` regenerates it, and
`traksha/data/fixture/ATTRIBUTION.md` carries the swisstopo licence and the
...
def load_sample_scene(size: int = 384, sun: Optional[tuple] = None,
    ...
    meta = SceneMeta(
        ...
        source="swisstopo sample scene (see traksha/data/fixture/ATTRIBUTION.md)")
