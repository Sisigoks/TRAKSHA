...
Phase 2 has `traksha dataset` and a `dataset.json` full of evidence. Delivery had
...
def run_delivery(
    ...
    work = tempfile.mkdtemp(prefix="traksha-delivery-", dir=work_root)
...
def write_report(rep: dict, path: str) -> str:
    ...
    A("")
    A("```bash")
    A("python -m traksha.cli delivery results/zurich")
    A("```")
    A("")
