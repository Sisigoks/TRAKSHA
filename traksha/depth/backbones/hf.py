...
surface and our own calibration on top of it (see traksha.chhaya).
...
`traksha.learn` - costs milliseconds. Carrying device selection, autocast and VRAM
...
class HFDepthBackbone(DepthBackbone):
    ...
    TRAKSHA is CPU-only by design, not by accident. The one component that could
    ...
    over a dataset, `traksha.learn` - is a handful of dot products. So the device
